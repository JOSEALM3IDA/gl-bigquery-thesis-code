...
CREATE OR REPLACE PROCEDURE cp_gain_loss_bachelor_thesis.calc_gain_loss_rp_fmcg_cp_ir(
  start_p1 DATE, end_p1 DATE, start_p2 DATE, end_p2 DATE)
BEGIN

WITH
table_raw_data_period AS (
    -- Once only: define each entry's period
    ...
    FROM (
        -- Once only: perform brand aggregation/selection + input raw fcmg data
        SELECT hhkey,
            CASE WHEN ba.brand IS NOT NULL THEN ba.aggregation ELSE 'REST' END AS brand,
            value, brandfactor, rwkorr, rwstd, fullmasW, datep
        ...
        WHERE datep >= start_p1 AND datep <= end_p1 OR datep >= start_p2 AND datep <= end_p2 -- Push down date filter
    )
),
table_raw_households AS (
    -- Once only: input raw hh data
    SELECT hhkey, continuo -- Only input necessary fields
...
table_hh_brand_period AS (
    ...
    FROM table_raw_data_period trd
...
CALL cp_gain_loss_bachelor_thesis.calc_gain_loss_rp_fmcg_cp_ir(d_start_p1, d_end_p1, d_start_p2, d_end_p2);